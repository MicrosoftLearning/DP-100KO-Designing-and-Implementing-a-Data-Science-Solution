...
이 랩을 시작하기 전에 [랩 1A](Lab01A.md) 및 [랩 1B](Lab01B.md)를 완료해야 합니다. 이 두 랩에는 이 랩에서 사용되는 Azure Machine Learning 작업 영역 및 기타 리소스를 만드는 작업이 포함되어 있습니다. 그런 다음, 다음 단계를 수행하여 이 랩에 필요한 컴퓨팅을 초기화합니다.

1. [Azure Machine Learning Studio](https://ml.azure.com) 내 **컴퓨팅** 페이지의 **컴퓨팅 클러스터** 탭에서 이전에 생성한 컴퓨팅 클러스터의 이름을 클릭합니다.
2. 컴퓨팅 클러스터를 편집하여 **최소 노드 수**를 2(최소 및 최대 노드 수는 2)로 변경하고 **업데이트**를 클릭합니다. 이렇게 하면 클러스터 노드가 항상 실행되고 있으며 시작될 때까지 기다려야 하는 시간을 최소화합니다.
...
6. 데이터 스키마를 검토합니다. 여러 열의 분포가 히스토그램으로 표시됩니다. 그런 다음 시각화를 닫고, X 또는 **<sub>↗</sub><sup>↙</sup>** 아이콘을 사용하여 설정 창을 닫거나 최소화합니다. 그러면,데이터 세트가 있는 파이프라인 캔버스를 볼 수 있습니다.
...
2. **데이터 일반화** 모듈을 캔버스의 **diabetes dataset** 모듈 아래로 끕니다. 그런 다음 **당뇨병 데이터 세트** 모듈의 출력을 **데이터 일반화** 모듈의 입력에 연결합니다.
...
2. 우리가 훈련 중인 모델은 **당뇨병** 값을 예측할 것이기 때문에, **모델 학습** 모듈을 선택한 다음 해당 설정을 수정하여 **레이블 열**을 **당뇨병**으로 설정합니다(대소문자와 철자 정확히 일치!).
...
5. 모델의 성능을 평가하려면 유효성 검사 데이터 세트의 점수를 매겨서 생성된 몇 가지 메트릭을 확인해야 합니다. **모델 점수 매기기 및 평가** 섹션에서 **모델 평가** 모듈을 **모델 점수 매기기** 모듈의 캔버스로 끌어옵니다. 그런 다음 **모델 점수 매기기** 모듈의 출력을 **모델 평가** 모듈의 **데이터 세트 점수 매기기**(왼쪽) 입력에 연결합니다.
...
2. 우측 상단에서 **제출**을 클릭합니다. 그런 다음 메시지가 표시되면 **visual-training** *실험*을 새로 만들어 실행합니다.  그러면 컴퓨팅 클러스터가 초기화되고 파이프라인이 실행되는데, 이 과정은 10분 이상 걸릴 수 있습니다. 파이프라인 실행 상태는 디자인 캔버스 오른쪽 위에서 확인할 수 있습니다.
