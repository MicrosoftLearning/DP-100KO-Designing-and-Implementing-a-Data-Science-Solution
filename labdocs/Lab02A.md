...
그런 다음 작업 영역의 [Azure Machine Learning Studio](https://ml.azure.com) 내 **컴퓨팅** 페이지 **학습 클러스터** 탭에서 이전에 만든 **aml-cluster** 컴퓨팅 대상을 열고 **편집**을 클릭합니다. 그러고 나서 **최소 노드 수** 설정을 **1**로 지정하고 **업데이트**를 클릭합니다. 이렇게 하면 랩을 진행하는 동안 파이프라인을 실행하는 데 걸리는 시간을 줄일 수 있습니다.
...
3. 파이프라인을 실행할 컴퓨팅 대상을 지정해야 합니다. **설정** 창에서 **컴퓨팅 대상 선택**을 클릭하고 이전 랩에서 만든 **aml-cluster** 컴퓨팅 대상을 선택합니다.
4. Designer의 왼쪽에서 **데이터 세트** 섹션을 확장한 다음 이전 연습에서 만든 **diabetes dataset** 데이터 세트를 캔버스로 끕니다.
5. 캔버스에서 **diabetes dataset** 모듈을 선택하여 해당 설정을 확인합니다. 그런 다음 **출력** 탭에서 세로 막대형 차트 모양의 **시각화** 아이콘을 클릭합니다.
...
1. 왼쪽 창에서 **데이터 변환** 섹션을 확장합니다. 이 섹션에는 모델 학습 전에 데이터를 변환하고 전처리하는 데 사용할 수 있는 광범위한 모듈이 포함되어 있습니다. **데이터 일반화** 모듈을 캔버스의 **diabetes dataset** 모듈 아래로 끕니다. 그런 다음 **diabetes dataset** 모듈의 출력을 **데이터 일반화** 모듈의 입력에 연결합니다.
2. **데이터 일반화** 모듈을 선택하여 해당 설정을 확인합니다. 설정에서 변환 방법과 변환할 열을 지정해야 합니다. 그런 다음 변환을 **ZScore**로 유지하고 열을 편집하여 다음 열 이름을 포함합니다.
    * PlasmaGlucose
...
3. 이제 데이터를 학습과 유효성 검사를 위한 개별 데이터 세트로 분할할 수 있습니다. 왼쪽 창의 **데이터 변환** 섹션에서 **데이터 분할** 모듈을 캔버스의 **데이터 일반화** 모듈 아래로 끕니다. 그런 다음 **데이터 일반화** 모듈의 *변환된 데이터 세트*(왼쪽) 출력을 **데이터 분할** 모듈의 입력에 연결합니다.
4. **데이터 분할** 모듈을 선택하고 해당 설정을 다음과 같이 구성합니다.
    * **분할 모드**: 행 분할
...
1. 파이프라인이 다음과 같이 표시되는지 확인합니다. 이미지에는 각 모듈이 수행하는 작업을 설명하는 주석이 포함되어 있습니다. 실제 프로젝트에서 Designer를 사용할 때도 이와 같이 주석을 포함하는 것이 좋습니다.

    ![시각적 학습 파이프라인](images/visual-training.jpg)

2. 오른쪽 위에서 **실행**을 클릭합니다. 그런 다음 메시지가 표시되면 **visual-training** *실험*을 새로 만들어 실행합니다.  그러면 컴퓨팅 대상이 초기화되고 파이프라인이 실행됩니다. 이 과정은 10분 이상 걸릴 수 있습니다. 파이프라인 실행 상태는 디자인 캔버스 오른쪽 위에서 확인할 수 있습니다.

    **팁**: 실행 중인 파이프라인과 만들어진 실험은 **파이프파인** 및 **실험** 페이지에서 확인할 수 있습니다. 실험이 완료되면 **Designer** 페이지의 **시각적 당뇨병 학습** 파이프라인으로 다시 전환합니다.

3. **데이터 일반화** 모듈이 완료되어 &#x2705; 아이콘이 표시되면 해당 모듈을 선택하고 **설정** 창 **출력 + 로그** 탭의 **변환된 데이터 세트** 섹션 내 **포트 출력** 아래에서 **시각화** 아이콘을 클릭합니다. 그러면 변환된 출력의 통계 및 분포 시각화를 확인할 수 있습니다.
